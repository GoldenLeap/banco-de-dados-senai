...
INSERT INTO LIVROS(Nome_Livro, Editora, Autor, Genero, Quantidade, Preco) VALUE(
	"Alchemised", "Intrínseca" , "Sen Lin Yu", "Fantasia", 20, 144.42

);
# STrazer quantidade de livros
SELECT COUNT(*) AS Total_Livros
FROM LIVROS;

INSERT INTO Clientes (Nome_Cliente, CPF, Email, Telefone, Data_Nascimento) VALUES
("Joseph Icadura", "400.289.224-00", "joseph.icadura@email.com", '(50) 95555-6789', '1992-11-23');

INSERT INTO Vendas (Quantidade, Valor_Total, Data_Venda, CPF, ID_Livro) VALUEs
	(2, 288.84, '2025-09-29 9:00:00', '400.289.224-00', 6);
    
# Consultar livros que começam com a letra a e preço acima de 100.00
SELECT * FROM LIVROS WHERE Nome_Livro LIKE "A%" AND Preco >= 100.00;

# demonstrar a soma dos livros vendidos
SELECT SUM(Quantidade)  AS "Livros Vendidos"
FROM VENDAS;

# demonstrar a quantidade dos livros em estoque
SELECT SUM(Quantidade) AS "Livros em estoque" FROM LIVROS;

# apagar o livro com codigo 5
...
# Relatorio (Livros mais vendidos)
SELECT Vendas.Quantidade, Livros.Nome_Livro AS "Livros_Mais Vendidos"
FROM Vendas
INNER JOIN Livros ON Vendas.ID_Livro=Livros.ID_Livro
